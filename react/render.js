  ...
  if (component.default) {
    ...
    $(".script").each((i, el) => {
      ...
      if (data.jsLibs) {
        data.jsLibs
          .split(",")
          .forEach((lib) => !jsLibs.includes(lib) && jsLibs.unshift(lib));
      }
      if (data.jsLibs) {
        data.cssLibs
          .split(",")
          .forEach((lib) => !cssLibs.includes(lib) && cssLibs.unshift(lib));
      }
    ...
    if (jsLibs.length) {
      jsLibs.forEach((jsLib) => $("body").append(`<script src="${jsLib}" />`));
    }

    if (code.length) {
      code.forEach((jsCode) => $("body").append(`<script>${jsCode}</script>`));
    }
